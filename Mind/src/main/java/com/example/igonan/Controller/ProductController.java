...
*/
    ...
    @ResponseBody
    public Object productType(@PathVariable String productType)throws IOException {
        int prType = 1;
        ...
        switch (productType){
            ...
            case "etc" :
                ...
                break;
            default:
                if (productType != "tag" || productType != "chip" || productType != "etc"){
                    //return "<script>alert('유효 하지 않은 주소');</script>";
    ...
    @RequestMapping(value = "/main/productlist", method = { RequestMethod.POST }) //데이터가 보내지는 주소와 메소드 설정
    @ResponseBody // 자바 객체를 HTTP 응답 본문의 객체로 변환
    public Object mainProductList(){
        //상품의 전체 리스트를 db에서 가져온다
        List<ProductDTO> list = productService.getMainProductList();// List에 select 결과 list를 담음
    ...
    public Object paymentproductOne(@PathVariable("param")int parameter){
        List<ProductDTO> list = productService.getOneProduct(parameter); // List에 select 결과 list를 담음
