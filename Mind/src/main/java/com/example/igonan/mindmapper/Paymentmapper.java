...
@Select("select * from ubuy u where u.u_name = #{buyername} order by u.u_num desc limit #{startnum}, #{endnum} ;")
//"select * from ubuy u where u.u_name = #{buyername} order by u.u_date asc limit #{startnum}, #{endnum} ;"
List<PaymentDTO> mypageuserbuylist(String buyername, int startnum, int endnum);
...
//@Select("select u.u_name,count(*),sum(u_totalpay) from ubuy u where u.u_id = #{buyerid};")
 ...
 Integer mindpaymentinsert(String name, String phone, String addr, String saddr, String prName, int totalPay, String memo, int count, String cashsel, LocalDate buydate);
